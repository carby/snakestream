...
- [ ] 1.1 Add the `executor: Executor | None = None` parameter to
...
- [ ] 1.2 Rewrite `_derive()`'s docstring to carry both derivation rules: that
...
- [ ] 2.1 Reduce `sequential()` to `return self._derive(executor=SEQUENTIAL)`
      and `parallel()` to `return self._derive(executor=RACING)`.
- [ ] 2.2 Trim `sequential()`'s docstring to the two rules only it records —
      ...
      still points at it.
- [ ] 2.3 Grep `src/` and `tests/` for any remaining write to a `_executor` or
      `_chain` attribute outside `_derive()`; there should be none.

## 3. Verify

- [ ] 3.1 `uv run pytest` — full suite green, with no test edits. If any test
      required a change, stop: this change is specified as behaviour-preserving
      and a required edit means it is not.
- [ ] 3.2 `uv run ruff check .`, `uv run ruff format --check .`, and
      `uv run ty check src` all clean.
- [ ] 3.3 `uv run pytest --cov-fail-under=98` passes, matching the CI gate.
- [ ] 3.4 Retire the roadmap tripwire the 2026-08-26 `collapse-derive-wrappers`
...
- [ ] 3.5 Confirm the CLAUDE.md paragraph describing `.parallel()` /
