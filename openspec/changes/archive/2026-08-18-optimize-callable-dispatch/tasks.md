...
- [x] 1.1 Add `is_async_callable(fn) -> bool` to `callable_dispatch.py`:
...
- [x] 1.2 Keep `_maybe_await` as-is for per-composition call sites; do not
      delete it.
- [x] 1.3 Decide and document the canonical inline shape (a short comment in
...
- [x] 2.1 `filter()` predicate (`stream.py:144`).
- [x] 2.2 `map()` mapper (`stream.py:153`).
- [x] 2.3 `peek()` consumer (`stream.py:204`).
- [x] 2.4 `_collect_mutable()` accumulator (`stream.py:243`). Leave the
      `supplier` at line 241 on `_maybe_await` — it is invoked once per
      composition.
- [x] 2.5 `reduce()` accumulator (`stream.py:267`).
- [x] 2.6 `for_each()` consumer (`stream.py:273`).
- [x] 2.7 `for_each_ordered()` consumer (`stream.py:279`).
- [x] 2.8 `_min_max()` comparator (`stream.py:307`) — hoist above the
      `async for`, keeping the existing `check_comparator_result_type(sign)`
      guard unchanged.
- [x] 2.9 `_match()` predicate (`stream.py:331`), covering `all_match`,
      `any_match` and `none_match`.

## 3. collector.py call sites (15)

- [x] 3.1 `summing_int`/`summing_long`/`summing_double` mappers
      (`collector.py:79,91,103`), preserving `summing_double`'s `float()`
      coercion.
- [x] 3.2 `averaging_int`/`averaging_long`/`averaging_double` mappers
      (`collector.py:116,130,144`).
- [x] 3.3 `min_by`/`max_by` comparator (`collector.py:153`), keeping the
      `check_comparator_result_type` guard unchanged.
- [x] 3.4 `reducing` mapper and binary operator (`collector.py:217,221`).
...
- [x] 3.5 `to_map` key mapper, value mapper and merge function
...
- [x] 3.6 `grouping_by` classifier (`collector.py:269`).
- [x] 3.7 `partitioning_by` predicate (`collector.py:283`), preserving the
      `bool(...)` coercion.

## 4. sort.py call site (1, invoked per comparison)

- [x] 4.1 `_merge()` comparator (`sort.py:25`). `_merge` is called repeatedly
...
- [x] 5.1 Extend `tests/test_callable_dispatch.py` with direct
      ...
      wrapping each.
- [x] 5.2 Add the sync-`__call__`-returning-a-coroutine regression test — a
...
- [x] 5.3 Add a classification-does-not-leak-across-compositions test:
...
- [x] 5.4 Add `ParallelStream` coverage: an async-`__call__` mapper across
      racing branches still produces awaited values.
- [x] 5.5 Re-run the existing async-callable-object suite unchanged — every
      case in `tests/test_callable_dispatch.py` must still pass without
      modification.
- [x] 5.6 Verify the coverage gate still passes at `--cov-fail-under=98`. The
...
- [x] 6.1 Re-run the benchmark from `design.md` against the implemented
...
- [x] 6.2 `uv run pytest`, `uv run ruff check .`, `uv run ruff format --check .`,
      `uv run ty check src`.

## 7. Docs

- [x] 7.1 Update README's migration log: the dispatch contract narrows from
      per-result to per-callable-per-composition, tracked as **BREAKING** per
      `CLAUDE.md`'s convention.
- [x] 7.2 Update roadmap.md: move this item to **Done** with a summary and the
      measured numbers.
