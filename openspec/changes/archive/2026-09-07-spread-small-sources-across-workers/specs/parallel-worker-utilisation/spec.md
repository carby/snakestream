...
opposite used to be true: a small source was drained into a single worker's
batch, which made `.parallel()` slower than `.sequential()` on it and forced
callers to reason about batch counts before choosing an execution mode.
