...
- [x] 1.1 In `stream.py`, replace `_derive_executor()`'s body
  `return cast("Stream[T]", self._derive(self._chain, executor))` with
  `return self._derive(self._chain, executor)`.
- [x] 1.2 Confirm the `cast` import is still used — `iterate()` and the 3-arg
  `collect()` — and leave it in place.
- [x] 1.3 Run `uv run ty check src`. It must pass; `cast` is erased at runtime,
  so no test can distinguish the two forms and `ty` is the only gate.
- [x] 1.4 Run `uv run pytest` and `uv run ruff check . && uv run ruff format --check .`.
  Commit this on its own — it is correct independently of everything below.

## 2. Collapse the three methods into one

- [x] 2.1 Change `_derive`'s signature to `def _derive(self, op: Op | None = None) -> Stream[Any]`
...
- [x] 2.2 Move `_extend`'s docstring intent onto `_derive`: the chain-extension
  rule now lives in this body and nowhere else.
- [x] 2.3 Delete `_extend()` and rewrite its nine call sites as
  `self._derive(<Op>)` — `filter`, `map`, `flat_map`, `sorted`, `distinct`,
  `peek`, `limit`, `skip`, `unordered`. Each stays a one-liner.
- [x] 2.4 Delete `_derive_executor()` and rewrite `sequential()` / `parallel()`
  as derive-then-assign: `derived = self._derive()`, `derived._executor =
  SEQUENTIAL` / `RACING`, `return derived`.
- [x] 2.5 `grep -n "_extend\|_derive_executor" src/` returns nothing.

## 3. Re-site the immutability warning

- [x] 3.1 Put the full "must not assign onto self and return self" paragraph on
  `sequential()`, since the new body is a one-line edit away from the forbidden
  in-place flip (design.md Decision 4).
- [x] 3.2 Keep the "must not compose" warning in shortened form — the body now
...
- [x] 3.3 Make `parallel()`'s docstring point at `sequential()` rather than
...
- [x] 4.1 `uv run pytest` green with **no test file edited**: `git diff -- tests/`
...
- [x] 4.2 Confirm specifically that the position-independence tests for
...
- [x] 4.3 `uv run ruff check .`, `uv run ruff format --check .`,
  `uv run ty check src`, `uv run pytest --cov-fail-under=98`.
- [x] 4.4 `openspec validate collapse-derive-wrappers --strict`.
- [x] 4.5 No benchmark run needed, and say so in the commit message: this is
...
- [x] 5.1 `CLAUDE.md:61` — rewrite the `.parallel()`/`.sequential()` sentence so
...
- [x] 5.2 `roadmap.md:1195` (2026-08-24 **Done**) — annotate the sentence "so it
...
- [x] 5.3 `roadmap.md:727-745` (2026-08-25 **Done**, finding (a)) — annotate the
