...
`Stream.unordered()` SHALL set the instance's ordering flag to `False`
and return `self`, following the same mutate-and-return-self convention as
other chainable `Stream` methods (e.g. `filter()`, `map()`,
`on_close()`).
...
`Stream.sequential()` and `Stream.parallel()` SHALL propagate the
