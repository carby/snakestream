...
- [ ] 1.1 Add a `Characteristics` enum to `collector.py` with the single member
...
- [ ] 1.2 Add module-level `_NO_CHARACTERISTICS: frozenset[Characteristics] =
      ...
      the same reason.
- [ ] 1.3 Add `characteristics` as `Collector.__init__`'s fifth parameter,
      ...
      state.
- [ ] 1.4 Extend `Collector`'s class docstring: it currently says the type
...
- [ ] 1.5 **State in the `Characteristics` docstring that nothing in the
...
- [ ] 2.1 `to_set()` declares `UNORDERED`. Comment that it is the only factory
...
- [ ] 2.2 `mapping()` passes `characteristics=downstream.characteristics`.
      ...
      downstream's.
- [ ] 2.3 `collecting_and_then()` passes `characteristics=
...
- [ ] 2.4 Add a comment at **both** `grouping_by()` and `partitioning_by()`
...
- [ ] 2.5 Leave every other factory undeclared, and add one comment — at
...
- [ ] 3.1 `tests/test_collector.py`: the `Characteristics` scenarios — the
...
- [ ] 3.2 `tests/test_collector.py`: construction accepts a `list`, a `set` and
      a `frozenset` and stores a `frozenset` in each case.
- [ ] 3.3 `tests/test_collect.py` (or `test_collector.py`, wherever the
      ...
      being undone later.
- [ ] 3.4 `tests/test_mapping.py`: `mapping(len, to_set())` reports
      `UNORDERED`, `mapping(len, to_list())` does not, and
      `mapping(len, mapping(str, to_set()))` derives through both levels.
- [ ] 3.5 `tests/test_collecting_and_then.py`: `collecting_and_then(to_set(),
...
- [ ] 3.6 `tests/test_grouping_by.py` and `tests/test_partitioning_by.py`:
...
- [ ] 4.1 README: extend the `Collector(...)` prose at line 141 to the
...
- [ ] 4.2 README: state plainly that no operation consults the characteristic
...
- [ ] 4.3 CLAUDE.md: the Collectors section describes `Collector` as "a
...
- [ ] 4.4 roadmap.md: annotate the **Next** item-4 entry with what landed, and
...
- [ ] 5.1 `uv run ruff check .` and `uv run ruff format --check .` — expect
      B008 to be the rule task 1.2 is written against.
- [ ] 5.2 `uv run ty check src`, including that `characteristics` normalizes to
      `frozenset[Characteristics]` from every accepted input type.
- [ ] 5.3 `uv run pytest` and `uv run pytest --cov-fail-under=98`. If any new
      line needs a `pragma: no cover`, the field is under-specified rather than
      untestable — add the scenario instead of the pragma.
- [ ] 5.4 `openspec validate add-collector-characteristics --strict`.
