...
- [ ] 1.1 Move the ordering fold out of `Stream._is_ordered()` into a
...
- [ ] 1.2 Reduce `Stream._is_ordered()` to a one-line delegation, keeping the
...
- [ ] 1.3 Add a `is_ordered_at(chain, i)` form (or an index argument) giving the
  fold over `chain[:i]`, which is what the split rule needs.
- [ ] 1.4 Run `uv run pytest tests/test_unordered.py tests/test_for_each_ordered.py`
  and confirm no behaviour changed; this group is pure relocation.

## 2. Declare order-sensitivity on the Op protocol

- [ ] 2.1 Add an `order_sensitive: ClassVar[bool] = False` declaration to `Op` in
...
- [ ] 2.2 Set it `True` on `_LimitOp`, `_SkipOp` and `_DistinctOp` in
...
- [ ] 2.3 Extend `tests/test_op_protocol.py` to pin the default and the three
  overrides, and that no other op declares it.

## 3. The split rule

- [ ] 3.1 Implement the split-point search in `src/snakestream/execution.py`: the
...
- [ ] 3.2 Unit-test the search directly against representative chains, including
  ...
  limit.
- [ ] 3.3 Confirm that a chain with no split point returns `None` for every
  purely order-preserving chain and for every stateful op sitting at an unordered
  position.

## 4. Group-yielding branches

- [ ] 4.1 Add the group-yielding variant of `stream_through()` that yields
...
- [ ] 4.2 Flush anything emitted at `end()` as a final group ordered after every
  real one, so a head op that emits on `end()` is not lost.
- [ ] 4.3 Teach `_guarded()` to assign the source index under the lock and hand it
  on with the element.
- [ ] 4.4 Verify the plain `stream_through()` path is untouched and still used by
  `Sequential.elements()` and by every branch when there is no split point.

## 5. The reorder merge

- [ ] 5.1 In `race_through()`, when a split point exists, run the head ops in the
  branches and hold arriving groups in a dict keyed by index, releasing while the
  next expected index is present.
- [ ] 5.2 Drive the tail ops as one sequential sink chain over the released
...
- [ ] 5.3 Build the shared state map from the whole chain as today; head ops still
  share state across branches, tail ops are built once.
- [ ] 5.4 Confirm `race_through()` takes today's code path exactly when there is
  no split point.

## 6. Bounded read-ahead

- [ ] 6.1 Add the window object (last released index plus an `asyncio.Event`)
  shared between the merge and `_guarded()`, and the module-level `W` constant —
  not exported, per design Decision 4.
- [ ] 6.2 In `_guarded()`, await the window *before* competing for the lock and
  re-check after acquiring it; never wait while holding the lock.
- [ ] 6.3 Have the merge bump the released index and set the event on each
  release.
- [ ] 6.4 Test: a source far longer than `W` whose first element is far slower
  than the rest completes, and the number of elements pulled ahead of the first
  release stays within the window.
- [ ] 6.5 Test: closing the composed generator while a branch is blocked on the
  window does not hang — `aclose()` returns and no task is left uncancelled.

## 7. Cancellation across the barrier

- [ ] 7.1 Confirm cancellation raised by a tail `limit()` reaches the upstream
...
- [ ] 7.2 Test: an ordered racing `.limit(n)` over an infinite source yields
  exactly `n` elements in encounter order and terminates.
- [ ] 7.3 Test: the shared source is closed exactly once, and a second branch
...
- [ ] 8.1 `tests/test_limit.py`: the roadmap's reproduction — `range(12)`, a map
  slow for the first five, `.parallel().limit(5)` yields `[0, 1, 2, 3, 4]`.
- [ ] 8.2 `tests/test_skip.py`: the same source with `.skip(5)` drops `0..4`.
- [ ] 8.3 `tests/test_sorted.py`: an **async** source yielding `12..1` with
...
- [ ] 8.4 `tests/test_distinct.py`: equal-but-distinguishable objects — the
  survivor is the earliest in encounter order.
- [ ] 8.5 For each of the four, the mirror case: with `.unordered()` queued
  before the op, the order-blind result is produced and no ordering machinery
  runs.
- [ ] 8.6 Each of the four with the op queued *before* `.parallel()`, confirming
  the executor governs the whole pipeline.

## 9. Repay the test debt (a condition of this change, not a follow-up)

- [ ] 9.1 Restate the four `sorted()`-restores tests in `tests/test_unordered.py`
...
- [ ] 9.3 Add a `test_for_each_ordered.py` test that notices when the
...
- [ ] 9.4 Re-run the three mutation inversions (`_is_ordered()` always `True`,
...
- [ ] 10.1 Benchmark the unordered racing path before and after; a measurable
  per-element regression there is a blocker, not a footnote.
- [ ] 10.2 Benchmark the ordered racing path against the sequential path on
  `.map(slow).limit(5)` to show the concurrency this change is for is actually
  kept, and record the figures.
- [ ] 10.3 Pick `W` from the read-ahead/latency trade-off on those benchmarks and
  record the figures beside the constant, the way `Sequential.value()`'s docstring
  records its own.

## 11. Specs and docs

- [ ] 11.1 Update `openspec/specs/stream-execution-model/spec.md`'s stale
...
- [ ] 11.2 Update `CLAUDE.md`'s execution section: the primitive list, and the
  unconditional "Racing does not preserve ordering" claim, which becomes
  conditional.
- [ ] 11.3 Update `README.md` wherever it repeats that claim.
- [ ] 11.4 Move the roadmap's **Now** item to **Done** with what was decided and
...
- [ ] 12.1 `uv run pytest` green, and `uv run pytest --cov-fail-under=98` clears.
- [ ] 12.2 `uv run ruff check .`, `uv run ruff format --check .` and
  `uv run ty check src` all clean.
- [ ] 12.3 `openspec validate --change order-stateful-ops-under-racing --strict`
  passes.
