...
### Requirement: BaseStream.iterator() exposes the composed pipeline without consuming it
`BaseStream.iterator()` SHALL compose the stream's currently-queued chain of intermediate operations (via the same `_compose()` mechanism used by terminal operations) and return the resulting `AsyncGenerator[T, None]` to the caller, without pulling any elements from it. The caller SHALL be responsible for driving iteration (e.g. via `async for`, direct `__anext__()` calls, or partial consumption).
