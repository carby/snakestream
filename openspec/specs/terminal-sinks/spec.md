...
handoff SHALL obtain an `AsyncGenerator` by composing the chain through the
generator bridge.

Collectors SHALL be plain callables taking a composed `AsyncGenerator`. The
collector interface SHALL be independent of how terminal operations execute.
...
- **THEN** the collector receives a composed `AsyncGenerator` and returns the result of consuming it
