...
- **WHEN** `Stream(["a", "b", "c"]).collect(joining())` is called
...
- **WHEN** `Stream(["a"]).collect(joining(", "))` is called
...
- **WHEN** `Stream([]).collect(joining(", "))` is called
- **THEN** the result is `""`
...
- **WHEN** `Stream(["a", 1, "c"]).collect(joining())` is called
