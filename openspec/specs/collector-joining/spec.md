...
`collector.py` SHALL provide a `joining(delimiter: str = "", prefix: str = "", suffix: str = "")` function that returns a collector — an `async def` callable accepting an `AsyncGenerator[str, None]` and returning a `str` — usable with `Stream.collect(collector)`. It SHALL support Java's three `Collectors.joining` overloads via default arguments: no-arg (empty delimiter/prefix/suffix), delimiter-only, and delimiter+prefix+suffix.
