...
- **WHEN** `Stream([1, 2, 3]).collect(counting())` is called
- **THEN** the result is `3`

#### Scenario: Empty stream counts to zero
- **WHEN** `Stream([]).collect(counting())` is called
...
- **WHEN** `Stream(["a", "bb", "ccc"]).collect(summing_int(len))` is called
- **THEN** the result is `6`

#### Scenario: summing_long behaves identically to summing_int
- **WHEN** `Stream(["a", "bb", "ccc"]).collect(summing_long(len))` is called
- **THEN** the result is `6`

#### Scenario: summing_int on an empty stream returns zero
- **WHEN** `Stream([]).collect(summing_int(len))` is called
- **THEN** the result is `0`

#### Scenario: async mapper is awaited
- **WHEN** `Stream([1, 2, 3]).collect(summing_int(async_double))` is called with an async mapper doubling its input
...
- **WHEN** `Stream([1, 2, 3]).collect(summing_double(lambda x: x))` is called
- **THEN** the result is `6.0` and is a `float`

#### Scenario: summing_double on an empty stream returns 0.0
- **WHEN** `Stream([]).collect(summing_double(lambda x: x))` is called
...
- **WHEN** `Stream([1, 2, 3, 4]).collect(averaging_int(lambda x: x))` is called
- **THEN** the result is `2.5`

#### Scenario: averaging_long behaves identically to averaging_int
- **WHEN** `Stream([1, 2, 3, 4]).collect(averaging_long(lambda x: x))` is called
- **THEN** the result is `2.5`

#### Scenario: averaging_double behaves identically to averaging_int
- **WHEN** `Stream([1, 2, 3, 4]).collect(averaging_double(lambda x: x))` is called
- **THEN** the result is `2.5`

#### Scenario: averaging on an empty stream returns 0.0
- **WHEN** `Stream([]).collect(averaging_int(lambda x: x))` is called
- **THEN** the result is `0.0`
