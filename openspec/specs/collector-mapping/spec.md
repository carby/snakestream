...
- **WHEN** `Stream(["a", "bb", "ccc"]).collect(mapping(len, to_list()))` is called
- **THEN** the result is `[1, 2, 3]`

#### Scenario: Async mapper is awaited
- **WHEN** `Stream([1, 2, 3]).collect(mapping(async_double, to_list()))` is called with an async mapper doubling its input
- **THEN** the result is `[2, 4, 6]`

#### Scenario: Empty stream yields the downstream's empty result
- **WHEN** `Stream([]).collect(mapping(len, to_list()))` is called
- **THEN** the result is `[]`

#### Scenario: Mapping composes with a reducing downstream
- **WHEN** `Stream(["a", "bb", "ccc"]).collect(mapping(len, counting()))` is called
