...
async def test_find_min_value_normal_input():
    ...
    # when
    it = await Stream(input_list).min(lambda x, y: x - y)
    # then
    assert it == 1
...
async def test_find_min_value_async_input():
    ...
    input_list = [1, 2, 3, 4, 5]
    # when
    it = await Stream(input_list).min(async_comparator)
    # then
    assert it == 1
...
async def test_find_min_value_custom_comparator():
    input_list = ["a", "bb", "ccc"]
    # when
    it = await Stream(input_list).min(lambda x, y: len(x) - len(y))
    # then
    assert it == "a"
...
async def test_find_min_value_object_comparator() -> None:
    ...
    it = await Stream(input_list).min(lambda x, y: x.id - y.id)
    # then
...
async def test_find_min_value_keeps_first_of_tied_elements():
    input_list = [("a", 5), ("b", 5)]
    # when
    it = await Stream(input_list).min(lambda x, y: x[1] - y[1])
    # then
    assert it == ("a", 5)
...
async def test_find_min_value_rejects_bool_comparator():
    ...
    with pytest.raises(TypeError):
        await Stream(input_list).min(lambda x, y: x > y)
...
async def test_find_min_value_rejects_async_bool_comparator():
    ...
    # when / then
    with pytest.raises(TypeError):
        await Stream(input_list).min(async_comparator)
...
async def test_ordered_racing_min_keeps_the_first_of_tied_elements(run):
    # when
    it = await Stream(TIE_SOURCE).parallel().map(overtaken).filter(lambda p: p[1] == 5).min(by_key)
...
async def test_ordered_racing_min_agrees_with_the_sequential_answer():
    # when
    racing = await Stream(TIE_SOURCE).parallel().map(overtaken).filter(lambda p: p[1] == 5).min(by_key)
    sequential = await Stream(TIE_SOURCE).map(overtaken).filter(lambda p: p[1] == 5).min(by_key)
    # then
