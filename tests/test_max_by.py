...
async def test_max_by_selects_largest_element() -> None:
    # when
    result = await Stream([3, 1, 2]).collect(max_by(lambda a, b: a - b))
...
async def test_max_by_empty_stream_returns_none() -> None:
    # when
    result = await Stream([]).collect(max_by(lambda a, b: a - b))
...
async def test_max_by_keeps_first_of_tied_elements() -> None:
    ...
    # when
    result = await Stream(input_list).collect(max_by(lambda x, y: x[1] - y[1]))
...
async def test_max_by_async_comparator_is_awaited() -> None:
    ...
    # when
    result = await Stream([3, 1, 2]).collect(max_by(async_comparator))
...
async def test_max_by_rejects_bool_comparator() -> None:
    ...
    with pytest.raises(TypeError):
        await Stream([3, 1, 2]).collect(max_by(lambda x, y: x > y))
...
async def test_ordered_racing_max_by_keeps_the_first_of_tied_elements(run) -> None:
    # when
    it = await Stream(TIE_SOURCE).parallel().map(overtaken).collect(max_by(by_key))
...
async def test_the_collector_form_agrees_with_the_stream_form() -> None:
    # when
    collected = await Stream(TIE_SOURCE).parallel().map(overtaken).collect(max_by(by_key))
    reduced = await Stream(TIE_SOURCE).parallel().map(overtaken).max(by_key)
