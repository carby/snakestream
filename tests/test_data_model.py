...
async def test_async_for_yields_what_iterator_yields() -> None:
    direct = [x async for x in Stream([1, 2, 3, 4]).map(lambda x: x * 2).filter(lambda x: x > 2)]
    via = [x async for x in Stream([1, 2, 3, 4]).map(lambda x: x * 2).filter(lambda x: x > 2).iterator()]
...
async def test_aiter_pulls_nothing_until_driven() -> None:
    seen: list[int] = []
    it = Stream([1, 2, 3]).peek(seen.append).__aiter__()
    assert seen == []
...
async def test_async_for_over_an_ordered_racing_stream_is_in_encounter_order() -> None:
    ...
    assert [x async for x in Stream(source).parallel().map(slow_head)] == source
...
def test_a_close_handler_runs_on_block_exit() -> None:
    calls: list[str] = []
    with Stream([1, 2, 3]).on_close(lambda: calls.append("closed")):
        assert calls == []
    assert calls == ["closed"]
...
def test_handlers_still_run_when_the_block_raises() -> None:
    ...
    with (
        ...
        Stream([1, 2, 3]).on_close(lambda: calls.append("closed")),
    ):
...
def test_repr_names_the_type_the_chain_and_the_mode() -> None:
    r = repr(Stream([1, 2, 3]).map(lambda x: x).filter(lambda x: True).parallel())
    assert r == "<Stream [map, filter] parallel>"


def test_repr_of_a_bare_sequential_stream() -> None:
    assert repr(Stream([1, 2, 3])) == "<Stream [] sequential>"
...
def test_repr_shows_the_stage_concat_seeds() -> None:
    ...
    assert repr(Stream.concat(Stream([1]).unordered(), Stream([2]))) == "<Stream [unordered] sequential>"


def test_repr_pulls_nothing() -> None:
    seen: list[int] = []
    repr(Stream([1, 2, 3]).peek(seen.append))
    assert seen == []
...
def test_an_implicit_truth_test_raises() -> None:
    with pytest.raises(TypeError):
        _takes_if_branch(Stream([1, 2, 3]))
    with pytest.raises(TypeError):
        _negated(Stream([1, 2, 3]))
...
async def test_adding_two_streams_concatenates_them() -> None:
    assert await (Stream([1, 2, 3]) + Stream([4, 5])).collect(to_list()) == [1, 2, 3, 4, 5]


@pytest.mark.asyncio
async def test_the_operator_matches_concat_exactly() -> None:
    added = Stream([1, 2]).parallel() + Stream([3]).unordered()
    concatenated = Stream.concat(Stream([1, 2]).parallel(), Stream([3]).unordered())
...
def test_the_operator_carries_both_operands_close_handlers() -> None:
    calls: list[str] = []
    a = Stream([1, 2]).on_close(lambda: calls.append("a"))
    b = Stream([3]).on_close(lambda: calls.append("b"))
    (a + b).close()
    assert calls == ["a", "b"]
...
async def test_the_operator_chains() -> None:
    ...
    total = Stream([1, 2]) + Stream([3]) + Stream([4, 5])
...
def test_synchronous_iteration_is_refused() -> None:
    seen: list[int] = []
    s = Stream([1, 2, 3]).peek(seen.append)
    with pytest.raises(TypeError):
        list(s)
    assert seen == []
...
def test_length_is_refused() -> None:
    with pytest.raises(TypeError):
        len(Stream([1, 2, 3]))  # type: ignore[arg-type]
...
def test_reversal_is_refused() -> None:
    with pytest.raises(TypeError):
        reversed(Stream([1, 2, 3]))  # type: ignore[call-overload]


def test_equality_remains_identity() -> None:
    seen_a: list[int] = []
    seen_b: list[int] = []
    a = Stream([1, 2, 3]).peek(seen_a.append)
    b = Stream([1, 2, 3]).peek(seen_b.append)
    same_stream_as_a = a
    assert (a == b) is False
