...
async def test_a_collector_with_no_combiner_is_not_partitioned() -> None:
    ...
    seq = await Stream(source).sequential().collect(to_list())
    par = await Stream(source).parallel().collect(to_list())
...
async def test_merge_follows_encounter_order_not_a_permutation() -> None:
    ...
    seq = "".join(await Stream(letters).sequential().collect(list, list.append, _concat_parts))
    par = "".join(await Stream(letters).parallel().collect(list, list.append, _concat_parts))
...
async def test_unordered_does_not_change_merge_order() -> None:
    ...
    seq = "".join(await Stream(letters).sequential().collect(list, list.append, _concat_parts))
    par = "".join(await Stream(letters).parallel().unordered().collect(list, list.append, _concat_parts))
...
async def test_sequential_never_invokes_the_combiner() -> None:
    ...
    result = await Stream(list(range(_N))).sequential().collect(list, list.append, combiner)
...
async def test_every_element_lands_in_exactly_one_partition_through_filter_and_flat_map() -> None:
    ...
    result = await (
        Stream(source).parallel().filter(lambda x: x % 2 == 0).flat_map(lambda x: Stream([x, x + 1000])).collect(to_list())
    )
...
async def test_hand_rolled_collect_combiner_is_invoked_and_matches_sequential() -> None:
    ...
    seq = await Stream(source).sequential().collect(dict, accumulate, combiner)
    merges_after_seq = merges
    par = await Stream(source).parallel().collect(supplier, accumulate, combiner)
...
async def test_reduce_one_arg_still_dispatches() -> None:
    result = await Stream([1, 2, 3, 4]).reduce(lambda a, b: a + b)
    assert result == 10


@pytest.mark.asyncio
async def test_reduce_two_arg_still_dispatches() -> None:
    result = await Stream([1, 2, 3, 4]).reduce(10, lambda a, b: a + b)
    assert result == 20
...
async def test_reduce_three_arg_dispatches_and_invokes_combiner() -> None:
    ...
    source = list(range(_N))
    result = await Stream(source).parallel().reduce(0, lambda acc, x: acc + x, combine)
...
async def test_reduce_three_arg_under_sequential_equals_two_arg() -> None:
    ...
    two_arg = await Stream(source).sequential().reduce(0, lambda acc, x: acc + x)
    three_arg = await Stream(source).sequential().reduce(0, lambda acc, x: acc + x, lambda a, b: a + b)
...
async def test_reduce_three_arg_parallel_equals_sequential_over_several_batches() -> None:
    ...
    seq = await Stream(source).sequential().reduce(0, lambda acc, x: acc + x, lambda a, b: a + b)
    par = await Stream(source).parallel().reduce(0, lambda acc, x: acc + x, lambda a, b: a + b)
...
async def test_reduce_three_arg_async_callables() -> None:
    ...
    source = list(range(_N))
    result = await Stream(source).parallel().reduce(0, accumulate, combine)
...
async def test_ordered_batches_over_an_empty_source_yield_nothing() -> None:
    ...
    result = await Stream([]).parallel().sorted().collect(to_list())
    assert result == []
