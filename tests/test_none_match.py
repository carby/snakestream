import asyncio

import pytest

from snakestream import Stream


@pytest.mark.asyncio
async def test_empty_stream() -> None:
    it = await Stream([]).none_match(lambda x: x > 5)
    assert it is True


@pytest.mark.asyncio
async def test_simple() -> None:
    it = await Stream([1, 2, 3, 2, 3, 1, 2]).none_match(lambda x: x > 5)
    assert it is True


@pytest.mark.asyncio
async def test_some_matches() -> None:
    it = await Stream([1, 2, 3, 2, 3, 1, 2, 5, 6, 7]).none_match(lambda x: x < 5)
    assert it is False


@pytest.mark.asyncio
async def test_simple_async() -> None:
    async def async_predicate(x: int) -> bool:
        await asyncio.sleep(0.01)
        return x > 5

    it = await Stream([1, 2, 3, 2, 3, 1, 2]).none_match(async_predicate)
    assert it is True


@pytest.mark.asyncio
async def test_simple_async_short_circuit() -> None:
    async def async_predicate(x: int) -> bool:
        await asyncio.sleep(0.01)
        return x < 5

    it = await Stream([1, 2, 3, 2, 3, 1, 2]).none_match(async_predicate)
    assert it is False


@pytest.mark.asyncio
async def test_all_matches() -> None:
    it = await Stream([1, 2, 3, 2, 3, 1, 2]).none_match(lambda x: x < 5)
    assert it is False
