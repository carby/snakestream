...
async def test_parallel_simple(int_2_letter) -> None:
    # when
    it = await Stream([1, 2, 3, 4, 1, 2, 3, 4]).parallel().map(lambda x: int_2_letter[x]).distinct().collect(to_list())
    # then
    assert len(it) == 4
    assert "a" in it
    assert "b" in it
...
async def test_parallel_is_faster_than_sequential() -> None:
    ...
    await Stream([1, 2, 3, 4, 1, 2, 3, 4]).parallel().map(sleep).distinct().collect(to_list())
    end_parallel = time.time()
    time_parallel = end_parallel - start_parallel

    start_sequential = time.time()
    await Stream([1, 2, 3, 4, 1, 2, 3, 4]).map(sleep).distinct().collect(to_list())
...
async def test_parallel_distinct_no_cross_branch_duplicates() -> None:
    ...
    # when
    it = await Stream(values).parallel().distinct().collect(to_list())
...
async def test_parallel_skip_drops_exactly_n_across_branches() -> None:
    # when
    it = await Stream(list(range(100))).parallel().skip(10).collect(to_list())
...
async def test_parallel_skip_state_fresh_across_separate_streams() -> None:
    ...
    await Stream(list(range(20))).parallel().skip(10).collect(to_list())
    ...
    second = await Stream(list(range(20))).parallel().skip(10).collect(to_list())
...
async def test_parallel_distinct_state_fresh_across_separate_streams() -> None:
    ...
    await Stream([1, 2, 3] * 10).parallel().distinct().collect(to_list())

    # when
    second = await Stream([1, 2, 3] * 10).parallel().distinct().collect(to_list())
...
async def test_parallel_over_source_with_real_await_empty_chain() -> None:
    ...
    it = await Stream(_agen_with_real_await(20)).parallel().collect(to_list())
...
async def test_parallel_over_source_with_real_await_nonempty_chain() -> None:
    ...
    it = await Stream(_agen_with_real_await(20)).parallel().map(lambda x: x * 2).collect(to_list())
...
async def test_parallel_limit_with_real_await_source_closes_safely() -> None:
    ...
    it = await Stream(_agen_with_real_await(50)).parallel().limit(10).collect(to_list())
...
async def test_parallel_downstream_processing_stays_concurrent_with_real_await_source() -> None:
    ...
    # when
    start = time.time()
    it = await Stream(_agen_with_real_await(8)).parallel().map(slow_map).collect(to_list())
...
async def test_parallel_applies_to_ops_declared_before_it(int_2_letter) -> None:
    ...
    # when
    started = time.time()
    it = await Stream(list(range(8))).map(slow).parallel().collect(to_list())
...
async def test_parallel_declared_late_still_produces_every_element(int_2_letter) -> None:
    ...
    it = await Stream([1, 2, 3, 4, 1, 2, 3, 4]).map(lambda x: int_2_letter[x]).distinct().parallel().collect(to_list())
    # then
    assert len(it) == 4
    assert "a" in it
    assert "b" in it
