...
async def test_map_async_callable_object() -> None:
    actual = await Stream([1, 2, 3]).map(AsyncCallableDouble()).collect(to_list())
    assert actual == [2, 4, 6]


@pytest.mark.asyncio
async def test_filter_async_callable_object() -> None:
    actual = await Stream([1, 2, 3, 4]).filter(AsyncCallablePredicate()).collect(to_list())
    assert actual == [2, 4]
...
async def test_peek_async_callable_object() -> None:
    ...
    actual = await Stream([1, 2, 3]).peek(consumer).collect(to_list())
    assert actual == [1, 2, 3]
...
async def test_reduce_async_callable_object() -> None:
    actual = await Stream([1, 2, 3]).reduce(0, AsyncCallableAccumulator())
    assert actual == 6
...
async def test_for_each_async_callable_object() -> None:
    consumer = _RecordingAsyncCallableConsumer()
    await Stream([1, 2, 3]).for_each(consumer)
    assert consumer.seen == [1, 2, 3]


@pytest.mark.asyncio
async def test_sorted_async_callable_object_comparator() -> None:
    actual = await Stream([3, 1, 2]).sorted(comparator=AsyncCallableComparator()).collect(to_list())
    assert actual == [1, 2, 3]


@pytest.mark.asyncio
async def test_min_async_callable_object_comparator() -> None:
    actual = await Stream([3, 1, 2]).min(AsyncCallableComparator())
    assert actual == 1


@pytest.mark.asyncio
async def test_max_async_callable_object_comparator() -> None:
    actual = await Stream([3, 1, 2]).max(AsyncCallableComparator())
    assert actual == 3


@pytest.mark.asyncio
async def test_all_match_async_callable_object() -> None:
    actual = await Stream([2, 4, 6]).all_match(AsyncCallablePredicate())
    assert actual is True


@pytest.mark.asyncio
async def test_any_match_async_callable_object() -> None:
    actual = await Stream([1, 3, 4]).any_match(AsyncCallablePredicate())
    assert actual is True


@pytest.mark.asyncio
async def test_none_match_async_callable_object() -> None:
    actual = await Stream([1, 3, 5]).none_match(AsyncCallablePredicate())
    assert actual is True
...
async def test_map_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).map(_SyncCallReturningCoroutine()).collect(to_list())
    assert actual == [2, 4, 6]
...
async def test_filter_sync_call_returning_coroutine() -> None:
    ...
    actual = await Stream([1, 2, 3, 4]).filter(_EvenPredicateReturningCoroutine()).collect(to_list())
    assert actual == [2, 4]


@pytest.mark.asyncio
async def test_summing_int_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(summing_int(_SyncCallReturningCoroutine()))
    assert actual == 12


@pytest.mark.asyncio
async def test_map_partial_wrapping_async_callable_object() -> None:
    actual = await Stream([1, 2, 3]).map(functools.partial(AsyncCallableDouble())).collect(to_list())
    assert actual == [2, 4, 6]
...
async def test_map_classification_does_not_leak_across_compositions() -> None:
    ...
    first = await Stream([1, 2, 3]).map(mapper).collect(to_list())
    second = await Stream([4, 5, 6]).map(mapper).collect(to_list())
    assert first == [2, 4, 6]
...
async def test_classification_is_not_repeated_per_element_under_fork_join(monkeypatch) -> None:
    ...
    calls = 0
    sequential = await Stream(values).map(_sync_double).filter(lambda x: True).collect(to_list())
    sequential_calls = calls

    calls = 0
    parallel = await Stream(values).parallel().map(_sync_double).filter(lambda x: True).collect(to_list())
...
async def test_parallel_async_callable_object_mapper() -> None:
    actual = await Stream([1, 2, 3, 4]).parallel().map(AsyncCallableDouble()).collect(to_list())
...
async def test_collect_mutable_sync_call_returning_coroutine() -> None:
    ...
    actual = await Stream([1, 2, 3]).collect(supplier, _SyncCallReturningCoroutineBiConsumer(), lambda a, b: a)
    assert actual == [1, 2, 3]


@pytest.mark.asyncio
async def test_reduce_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).reduce(0, _SyncCallReturningCoroutineAccumulator())
    assert actual == 6
...
async def test_for_each_ordered_sync_call_returning_coroutine() -> None:
    consumer = _SyncCallReturningCoroutineConsumer()
    await Stream([1, 2, 3]).for_each_ordered(consumer)
    assert consumer.seen == [1, 2, 3]


@pytest.mark.asyncio
async def test_min_sync_call_returning_coroutine_comparator() -> None:
    actual = await Stream([3, 1, 2]).min(_SyncCallReturningCoroutineComparator())
    assert actual == 1


@pytest.mark.asyncio
async def test_all_match_sync_call_returning_coroutine() -> None:
    actual = await Stream([2, 4, 6]).all_match(_SyncCallReturningCoroutinePredicate())
    assert actual is True


@pytest.mark.asyncio
async def test_summing_long_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(summing_long(_SyncCallReturningCoroutine()))
    assert actual == 12


@pytest.mark.asyncio
async def test_summing_double_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(summing_double(_SyncCallReturningCoroutine()))
    assert actual == 12.0


@pytest.mark.asyncio
async def test_averaging_int_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(averaging_int(_SyncCallReturningCoroutine()))
    assert actual == 4.0


@pytest.mark.asyncio
async def test_averaging_long_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(averaging_long(_SyncCallReturningCoroutine()))
    assert actual == 4.0


@pytest.mark.asyncio
async def test_averaging_double_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(averaging_double(_SyncCallReturningCoroutine()))
    assert actual == 4.0


@pytest.mark.asyncio
async def test_grouping_by_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3, 4]).collect(grouping_by(_SyncCallReturningCoroutinePredicate()))
    assert actual == {True: [2, 4], False: [1, 3]}


@pytest.mark.asyncio
async def test_partitioning_by_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3, 4]).collect(partitioning_by(_SyncCallReturningCoroutinePredicate()))
    assert actual == {True: [2, 4], False: [1, 3]}


@pytest.mark.asyncio
async def test_reducing_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2, 3]).collect(reducing(0, _SyncCallReturningCoroutineAccumulator()))
    assert actual == 6


@pytest.mark.asyncio
async def test_to_map_sync_call_returning_coroutine() -> None:
    actual = await Stream([1, 2]).collect(to_map(_SyncCallReturningCoroutinePredicate(), _SyncCallReturningCoroutine()))
    assert actual == {False: 2, True: 4}


@pytest.mark.asyncio
async def test_min_by_sync_call_returning_coroutine_comparator() -> None:
    actual = await Stream([3, 1, 2]).collect(min_by(_SyncCallReturningCoroutineComparator()))
    assert actual == 1


@pytest.mark.asyncio
async def test_sorted_sync_call_returning_coroutine_comparator() -> None:
    actual = await Stream([3, 1, 2]).sorted(comparator=_SyncCallReturningCoroutineComparator()).collect(to_list())
    assert actual == [1, 2, 3]
