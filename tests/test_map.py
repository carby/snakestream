...
async def test_map_does_not_mutate_source(int_2_letter) -> None:
    ...
    # when
    it = await Stream(source).map(lambda x: int_2_letter[x]).collect(to_list())
...
async def test_map_matches_builtin_map(values: list[int]) -> None:
    # when
    actual = await Stream(values).map(lambda x: x * 2).collect(to_list())
...
async def test_map_async_mapper_matches_builtin_map(values: list[int]) -> None:
    ...
    # when
    actual = await Stream(values).map(async_double).collect(to_list())
