...
1. [`UNSET` and `unseeded()` sit in `sink.py` on rejected reasoning](items/sink-sentinel-placement.md) — filed 2026-09-03<br>  **Gate:** a decision on whether `UnseededSink` earns the fold vocabulary its home despite `collectors.py`'s non-sink boxes — the import graph will not settle it, since the split leaves `UNSET`'s three importers unchanged
2. [`async with` on `Stream`](items/async-with-on-stream.md) — filed 2026-08-31<br>  **Gate:** a delta to the `stream-close-handling` capability first — `CloseHandler` widened to permit an awaitable and `close()` given an async twin — with `__aenter__`/`__aexit__` following from it, not bolted on ahead of it
3. [Sharing anything between the segment-sign twins measures ~3%](items/segment-sign-sharing-cost.md) — filed 2026-09-08<br>  **Gate:** a shape that de-duplicates without paying the ~3%, or a measurement on other hardware that overturns it - not a tidier version of the same call
