...
The append-only log of closed roadmap items: what shipped, and — more often
than the openspec archive can show — what was measured and **declined**. A
rejected change frequently has no archive directory at all, so this file is the
only place its reasoning survives. **Read it before proposing a cleanup**; the
answer to "has this been tried?" is here.

Entries are newest-first and are never edited once written, beyond dated
annotations marking in place a claim that later events falsified. The live
