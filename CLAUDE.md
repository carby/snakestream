...
Whether "on its own OS thread" means real CPU parallelism depends on the interpreter build: GIL-bound on the standard build (only one thread runs Python bytecode at a time, confirmed at ~1.0x), genuinely parallel on the free-threaded build (3.14t, PEP 779; ~2x measured on a large source, ~1.2x-2.1x on a small one — a source larger than `WORKERS` is guaranteed to be distributed across more than one worker, per `parallel-worker-utilisation`, rather than having its remainder drained into a single worker's second batch the way a one-step jump to the steady-state batch size used to). Cheap, non-blocking callables pay a new cost on either build — a real `asyncio.to_thread` dispatch per batch rather than one more cooperatively-scheduled `asyncio` task — which scales with the number of batch dispatches (roughly `source size / BATCH_SIZE` at steady state), not with worker count alone, which is why `.sequential()` remains the documented remedy for a pipeline where `.parallel()` was never buying anything (see CHANGELOG.md's migration entry, design.md's Risks, and `benchmark-findings.md`, all in `fork-join-executor-and-spliterator`).
...
`on_close()`/`close()` on `Stream` implement Java's AutoClose equivalent. `Stream` is itself a context manager, so `with stream as s:` is the idiom; `contextlib.closing()` still works and is what older examples use. Close handlers are plain no-arg callables — sync or async, like every other user-supplied callable in this library — not stream-aware; useful when subclassing `Stream` to wrap an I/O-like resource. `close()` stays synchronous: it runs a sync handler and refuses one whose result is awaitable (`StreamBuildException`, naming the handler and pointing at `aclose()`/`async with`), joining that refusal into the same per-handler failure list a raising handler produces rather than treating it as a special case. `aclose()` is the async twin — same handler list, same registration order, one at a time, but it awaits an awaitable result instead of refusing it — and `Stream` gains `__aenter__`/`__aexit__` alongside `__enter__`/`__exit__`, so `async with stream as s:` and `contextlib.aclosing(stream)` both work. Use `with`/`close()` when every handler is sync; `async with`/`aclose()` is required once one needs awaiting.
...
`Stream` satisfies the Python protocols whose Java counterparts it already claims — `__aiter__` for `BaseStream.iterator()`, `__enter__`/`__exit__` and `__aenter__`/`__aexit__` for `AutoCloseable`, `__repr__` for `toString()` — plus `__add__` as sugar over `Stream.concat`, the one member with no Java counterpart and a deliberate expansion of the 1:1 surface rather than a parity fix.
...
README.md tracks Java Stream API parity in detail (implemented / not-yet-implemented / intentionally-skipped methods). Check it before assuming a Java Stream method is or isn't implemented, and update it when adding or renaming public API surface. `CHANGELOG.md` is the migration log that used to live under README's `## Migration`: every breaking change gets an entry there in the same commit, newest first, saying whether the break is loud or silent. It is a migration log rather than a release log — new features, performance work and internal refactors stay out of it.
