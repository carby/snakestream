...
|---|---|---|
| 1 | **Specialize `_maybe_await`'s callable dispatch from per-result to per-callable** — `callable_dispatch.py` decides whether to await by calling `inspect.isawaitable` on the *result*, once per element per operation, across 30 call sites (`stream.py` 13, `collector.py` 15, `sort.py` 2). Classifying the *callable* once per composition instead reaches the speed of a version with no async support at all. | No blockers, no execution-model change, no public API change. Measured 2.8x: a chain of 8 `.map()` ops over 20,000 elements costs 5,775 ns/element today vs. 2,064 ns classified, against a 2,030 ns no-async-support floor (Python 3.14). Surfaced while sizing the Sink-chain item below, which measured 15–17% at far larger blast radius — this is both cheaper and substantially faster, and the two are independent. Carries one deliberate contract narrowing (a callable must be consistently sync or consistently async), specified rather than left implicit. See `openspec/changes/optimize-callable-dispatch`. |
