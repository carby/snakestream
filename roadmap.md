...
**None remain. Gaps 1 and 4 moved to Later on 2026-09-01; gaps 2 and 3 closed
together on 2026-09-01 as `add-collector-container-overloads`; gap 5 shipped
2026-09-01 as `add-comparator-null-ordering`** — the numbering
...
1. **Retired — `Stream.reduce(identity, accumulator, combiner)` moved to
   **Later** on 2026-09-01**, into the combiner row it was split out of on
   2026-08-31. The split was right about the *fact* it corrected (`reduce()`
   accepts no combiner at all, where `collect()`'s already ships inert) and
   wrong about the bucket that fact implied: it read the gap as unblocked
   because adding an inert parameter is easy, which is effort, not the entry
...
4. **Retired — `Comparator.thenComparing(Comparator)` moved to **Later** on
   2026-09-01.** It sat here on a premise that had degraded in transit. This
   entry used to say the overload "breaks the 'every segment yields a key'
   invariant", and README's `comparing(f, keyComparator)` row says the same;
   the two cited each other and neither cited the decision they were both
   paraphrasing. That decision —
   `openspec/changes/archive/2026-08-28-add-comparator-chaining/design.md`,
   "`then_comparing()` takes a key extractor or another `KeyComparator`, and
   nothing else" — says something weaker and more useful: a **sync** comparator
   segment *can* be folded into the tuple via `cmp_to_key`, an **async** one
   cannot, and it is that asymmetry that was declined. The invariant is a
   consequence of the decline, not its cause, and the overload is therefore
   blocked on an undecided trade rather than on effort or impossibility —
   **Later**'s entry criterion, not this bucket's. See **Later**.
5. **Retired — `Comparator.nullsFirst` / `nullsLast` shipped 2026-09-01**, as
   `add-comparator-null-ordering`. The gap closed both mechanisms a caller sees
   as one symptom: a `None` element, and a `None` extracted key, which Java
   reaches only through the declined `comparing(f, nullsFirst(...))` overload.
   Null tolerance is a comparator-level field, not a per-segment one — a
   `KeyComparator` from `nulls_first`/`nulls_last` keeps `sorted()`'s
   decorate-sort-undecorate fast path via a `(present, key)` column, rather
   than dropping to `cmp_to_key`. Does not touch gap 4's premise: see that
   entry's last sentence. The other four are parity for
...
| **`Comparator.thenComparing(Comparator)`** (`comparator.py`) — the bare-comparator tie-break overload, and with it the two overloads README records as decided against, `comparing(f, keyComparator)` and `thenComparing(f, keyComparator)`, which rest on the same premise and should be re-decided together rather than inherit a decline whose reason has moved. | **Moved here 2026-09-01 from Now → Queued changes**, where `enumerate-java-8-parity-gaps` filed it as gap 4. **The premise, from the origin rather than a paraphrase of it** (`openspec/changes/archive/2026-08-28-add-comparator-chaining/design.md`, "`then_comparing()` takes a key extractor or another `KeyComparator`, and nothing else"): `cmp_to_key` turns a **sync** comparator into a key extractor, so a sync comparator segment folds into the existing decorate-sort-undecorate tuple and rides all three direction lanes unchanged — verified 2026-09-01, a `cmp_to_key` object sorts inside a tuple and under `reverse=True`. An **async** comparator segment cannot: tuple comparison runs in C and cannot await. The decline bought off *that asymmetry*, not an impossibility. **The decision this needs** is what an async comparator segment does — rejected at construction (`is_async_callable` already classifies it, and `then_comparing()` is the natural place to raise), or a whole-chain fallback to `merge_sort` over `__call__`, which already handles async segments but silently drops the fast path. Nobody has taken it. **Two things to check before starting.** Python cannot tell a one-argument key extractor from a two-argument comparator by signature alone, so `then_comparing()` needs an arity check at construction, a distinct method name, or an explicit wrapper — the same obstacle that decided the two `keyComparator` rows. And the bool contract means a comparator segment must be wrapped in `_checked`, which lives in `sort.py` while `then_comparing()` lives in `comparator.py`; storing the raw comparator plus a marker and building `cmp_to_key(_checked(...))` lazily in `sort.py` keeps the split `split-sort-into-comparator-and-sort` settled. **Does not block gap 5**: null tolerance is a transform on a key an extractor already produced, so it needs no comparator segment — see `add-comparator-null-ordering`'s design.md, Decision 5. |
